	...
	public class FX_LinearToGamma
	{
		private struct FX_Job : IJobParallelFor
		{
			...
			public void Execute(int i) // i = pixel index in texture's raw data
			{
				...
				// if project is linear color space then convert it to gamma space on end
				A.r = Mathf.LinearToGammaSpace(A.r);
				A.g = Mathf.LinearToGammaSpace(A.g);
				A.b = Mathf.LinearToGammaSpace(A.b);
		...
		/// <summary>
		/// Returns a new Texture2D created from a 'texture' converted from Linear to Gamma color space.
		/// </summary>
