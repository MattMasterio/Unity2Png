...
namespace Unity2Png
{
	...
	/// </summary>
	public class FX_TransitionStandardNormalized
	{
		...
		public Texture2D Process(Texture2D textureA, Texture2D textureB, int transitionFrame, int transitionFramesCount, bool debug)
		{
			...
			float mod = (1f + (0.5f - Mathf.Abs(transition - 0.5f)) * 0.2f); // add 0%-20% (max in middle of transition)

			if(debug)
				Debug.LogError("Transition frame = " + (transitionFrame + 1).ToString("D2") + " / " + transitionFramesCount.ToString("D2") + ", transition = " + transition.ToString("F2") + ", mod = " + mod.ToString("F2") + ", alpha = " + alpha.ToString("F2"));
