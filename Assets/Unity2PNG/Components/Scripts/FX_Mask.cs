...
namespace Unity2Png
{
	...
	public class FX_Mask
	{
		private struct FX_Job : IJobParallelFor
		{
			...
			{
				// Color32 as Color
				Color A = A_RawData[i]; // texture
				Color B = B_RawData[i]; // mask

				// process pixels
				A.a = A.a * B.r;								// if mask is black and white (fast)
				//A.a = A.a * Mathf.Min(B.r, B.g, B.b);			// if mask is rgb (much slower)
		...
		/// <summary>
		/// Returns a new Texture2D created from a 'mask' applied on the 'texture'.
		/// </summary>
		public Texture2D Process(Texture2D texture, Texture2D mask)
		{
			var A_RawData = texture.GetRawTextureData<Color>();
			var B_RawData = mask.GetRawTextureData<Color>();
