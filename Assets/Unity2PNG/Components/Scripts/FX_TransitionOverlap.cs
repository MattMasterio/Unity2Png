...
namespace Unity2Png
{
	...
	/// </summary>
	public class FX_TransitionOverlap
	{
		...
		/// </summary>
		public Texture2D Process(Texture2D textureA, Texture2D textureB, int transitionFrame, int transitionFramesCount, bool debug)
		{
			...
			// custom data
			int half = transitionFramesCount / 2;
			float transition = Mathf.Clamp01((float)(transitionFrame + 1) / (float)(transitionFramesCount + 1));
			Vector2 alpha = new Vector2
			(
				Mathf.Clamp01(((float)transitionFrame + 1) / ((float)half + 1)),
				Mathf.Clamp01(1f - ((float)(transitionFrame + 1 - half) / ((float)half + 1)))
			);

			if (debug)
				Debug.LogError("Transition frame = " + (transitionFrame + 1).ToString("D2") + " / " + transitionFramesCount.ToString("D2") + ", transition = " + transition.ToString("F2") + ", alpha = " + alpha.ToString("F2"));
